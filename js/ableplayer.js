...
(function ($) {		//setup
	...
	window.AblePlayer = function(media) {
		...
		if ($(media).data('description-path') !== undefined && $(media).data('description-path') !== "false"){
			this.descriptionPath = $(media).data('description-path');
		}
		...
		$.when(this.getDescriptionText()).then(
		$.when(this.getDownloadText()).then(
...
(function ($) {		//default
	...
	AblePlayer.prototype.setDefaults = function () {
		...
		//JULIAN
		//OBTIENE LA RELACION DE ASPECTO QUE HEMOS ALMACENADO PREVIAMENTE EN UN ATRIBUTO DATA DE CADA FUENTE.
		//CON ESTO SABE SI TIENE QUE AÃ‘ADIR BANDAS NEGRAS EN EL VIDEO PARA QUE LOS SUBTITULOS NO SE SUPERPONGAN.
		this.aspectRatio = $("#video1").find("source").first().attr("data-aspect-ratio");
		
		if(this.aspectRatio){
			if(this.aspectRatio != '4:3' && this.aspectRatio != '16:9'){
				if(this.debug){
					console.log('data-aspect-ratio is defined, but is in a format currently not supported');
				}
			}
		}
	};
	...
	AblePlayer.prototype.getRootWebSitePath = function() { 
		...
		var htmlFolderIndex = _location.lastIndexOf("/");
		var htmlFolder = _location.substring(0, htmlFolderIndex);
		var rootWebIndex = htmlFolder.lastIndexOf("/");
		var rootWebFolder = htmlFolder.substring(0, rootWebIndex);
		return rootWebFolder;
	};
...
(function ($) {		//injects and adding elements
	AblePlayer.prototype.injectPlayerCode = function() { 
		...
		this.$ableDiv = this.$mediaContainer.wrap('<div class="able" tabindex="-1"></div>').parent();
...
text: '0:00'
	...
	AblePlayer.prototype.injectTextDescriptionArea = function () {
		...
		if(!this.prefClosedDesc || this.prefClosedDesc !== 1){
			this.$descDiv.hide();
		}
		else {
	...
	//	JULIAN
	//	CREA EL ENTORNO NECESARIO PARA EL ACORDEON Y AGRUPAR EN SU INTERIOR LOS TRES DIVS
	//	CON LA TRANSCRIPCION LA DESCRIPCION Y LAS DESCARGAS
	AblePlayer.prototype.injectAccordionArea = function () {
		this.$accordionDiv = $('<div>', {
			'id': 'accordion'
		});
		var $transcriptHeader = $('<h3></h3>');
		var $descriptDataHeader = $('<h3></h3>');
		var $downloadHeader = $('<h3></h3>');
		
		$transcriptHeader.text(this.tt.transcript);
		$descriptDataHeader.text(this.tt.descriptionH);
		$downloadHeader.text(this.tt.download);
		
		if(this.includeTranscript){ //SOLO SI TIENE TRANSCRIPCION
			this.$accordionDiv.append($transcriptHeader);
			this.injectTranscriptArea();
		}
		if(this.dt){	//SOLO SI TIENE DATOS DESCRIPTIVOS
			this.$accordionDiv.append($descriptDataHeader);
			this.injectDescriptDataArea();
		}
		if(this.lt){	//SOLO SI TIENE DESCARGAS
			this.$accordionDiv.append($downloadHeader);
			this.injectDownloadsArea();
		}
		
		
		if(this.$ableColumnRight){
			this.$ableColumnRight.prepend(this.$accordionDiv);
		}
		else {
			this.splitPlayerIntoColumns('accordion');
		}
		this.$accordionDiv.accordion({
collapsible: true,
heightStyle: "content",
		});
		//ES NECESARIO PARA DAR EL EFECTO DE ACORDEON
	}
	...
	AblePlayer.prototype.injectTranscriptArea = function() {
		...
		if (this.transcriptDivLocation) {
			...
			this.$accordionDiv.append(this.$transcriptArea);
		}
		...
		if (!this.prefTranscript && !this.transcriptDivLocation) { 
			this.$accordionDiv.hide(); 
		}
	};

	AblePlayer.prototype.injectDescriptDataArea = function () {
		/* JULIAN
SE ESTA USANDO FICHEROS .JS EN FORMATO JSON QUE ALMACENAN DATOS DESCRIPTIVOS ACERCA DEL VIDEO Y EL AUTOR.
SE APLICAN REGLAS PARA PERMITIR DEJAR CAMPOS VACIOS.
*/
		
		this.$descriptDataArea = $('<div>', {
			"class": "able-descript-data-area"
		});
		var $list = $('<ul></ul>');
		
		for(var x in this.dt){
			var index = this.dt[x];
			var $tag = $('<li> </li>');
			$tag.html(x + ': ' + index);
			if(index != ""){
		...
		//JULIAN
		//LO HE PUESTO DE ESTA FORMA YA QUE SI NO SE RELLENA UNO DE LOS CAMPOS NO PASA NADA
		//SOLAMENTE LO IGNORA. PROBABLEMENTE SEA INTERESANTE PLANTEARSE QUE CAMPOS QUEREMOS 
		//OBTENER Y PREPARAR EL JSON CONFORME A ESO. DE MOMENTO INCLUYE LO QUE YO HE CONSIDERADO
		//CAMPOS DE INTERESES GENERAL, DEJO A OBSERVACION EL INCLUIR MAS Y MEJORAR ESTO POR TANTO. 
		
		this.$descriptDataArea.append($list);
		
		if(this.dt){
			this.$accordionDiv.append(this.$descriptDataArea);
		}
	}

	AblePlayer.prototype.injectDownloadsArea = function () {
		/* JULIAN
SE ESTA AÃ‘ADIENDO UN ARCHIVO .JS QUE ALMACENA LAS RUTAS HASTA LOS ARCHIVOS DE INTERES, JUNTO CON UN NOMBRE AL QUE SE APLICARAN REGLAS PARA CONVERTIR A UN FORMATO LEGIBLE.
*/
		
		this.$downloadsArea = $('<div>', {
			'class': 'able-downloads-area'
		});
		
		var $list = $('<ul></ul>');
		var $descText = $('<p>');
		$descText.text(this.tt.descriptArea);
		this.$downloadsArea.append($descText);
		
		for(var x in this.lt) {
			var index = this.lt[x];
			var link = this.getRootWebSitePath() + '/' + index;
			var $li = $('<li></li>');
			var $tag = $('<a href="' + link + '" download> </a>');
			if(x.indexOf("captions") != -1) {
				var lang = x.substring((x.length -2), x.length);
				if (lang === 'En') {
					lang = this.tt.english;
				} else if (lang === 'Es') {
					lang = this.tt.spanish;
				}
				$tag.html(lang + " " + this.tt.captions);
			} 
			else if (x.indexOf("description") != -1) {
				var lang = x.substring((x.length -2), x.length);
				if (lang === 'En') {
					lang = this.tt.english;
				} else if (lang === 'Es') {
				...
				$tag.html(lang + ' ' + this.tt.descriptions);
			}
			else if (x.indexOf("Video") != -1) {
				var type = x.substring(0, (x.length - 5));
				$tag.html("Video " + type);
			}
			else if (x.indexOf("Sign") != -1) {
				var type = x.substring(0, (x.length - 4));
				$tag.html(this.tt.signVideo + ' ' + type);
			}
			else if (x.indexOf("Desc") != -1) {
				var type = x.substring(0, (x.length - 4));
				$tag.html(this.tt.videoDescriptions + ' ' + type);
			}
			else {
				//JULIAN
				//SI NO HAY UNA REGLA PARA APLICARSE, AÃ‘ADE EL ENLACE CON EL NOMBRE PUESTO EN EL ARCHIVO JSON.
				//EL USUARIO DEBERIA AÃ‘ADIR REGLAS PERSONALIZADAS PARA LOS ENLACES QUE HAYA ESCRITO EL.
				$tag.html(x);
			}
			$li.append($tag);
			$list.append($li);
		}
		this.$downloadsArea.append($list);
		if(this.lt){
			this.$accordionDiv.append(this.$downloadsArea);
		}
	}

	AblePlayer.prototype.splitPlayerIntoColumns = function (feature) { 
		// feature is either 'accordion' or 'sign' 
		...
		if (feature === 'accordion') {
			this.$accordionDiv.insertAfter(this.$ableColumnLeft);
			this.$ableColumnRight = this.$accordionDiv.wrap('<div class="able-column-right">').parent();
		}
...
top: this.$mediaContainer.offset().top
	...
	// JULIAN
	// AHORA TAMBIEN RECONOCE 'descriptions' PARA DECIDIR QUE DESCRIPCIONES QUEREMOS.
	...
	AblePlayer.prototype.setupPopups = function () {
		...
		if (popups.length > 0) { 
			...
			for (var i=0; i<popups.length; i++) {         
				...
				else if (popup == 'descriptions'){
					this.descriptionsPopup = this.createPopup('description');
					tracks = this.descriptions; 
				}
				var trackList = $('<ul></ul>');
				...
				for (j in tracks) {
					...
					if (popup == 'captions') { 
						...
						trackLabel.text(track.language);
						trackButton.click(this.handleDescriptionClick(track));
					}
				...
				if (popup == 'captions') {
					...
					//aÃ±ade un boton para quitarlas y para activar o desactivar las audiodescripciones.
					var audioOnButton = $('<input>', {
						'type': 'radio',
						'name': 'audioDesc',
						'id': 'audio-desc-on'
					});
					var audioOnLabel = $('<label>', {
						'for': 'audio-desc-on'
					}).text(this.tt.turnOnDescriptions);
					var textOffButton = $('<input>', {
						'type': 'radio',
						'name': radioName,
						'id': 'audio-desc-off'
					});
					var textOffLabel = $('<label>', {
						'for': 'audio-desc-off'
					}).text(this.tt.turnOffTextDescriptions);
					this.item1 = $('<li></li>').append(audioOnButton, audioOnLabel);
					this.item3 = $('<li></li>').append(textOffButton, textOffLabel);
					textOffButton.click(this.textDescriptionOff());
					audioOnButton.click(this.toggleAudioDescription());
					if(!this.prefDesc){
						this.item1.find("label").text(this.tt.turnOnDescriptions);
					}
					else {
						this.item1.find("label").text(this.tt.turnOffDescriptions);
					}
					...
					trackList.append(this.item1,  this.item3);
				}
				...
				if (popup == 'captions') {
					...
					this.descriptionsPopup.append(trackList);
				}
			}
		}    
	};
	...
	AblePlayer.prototype.calculateControlLayout = function () {
		...
		controlLayout['br'].push('options'); //YA NO FUNCIONA CON Preferences
	...
	AblePlayer.prototype.addControls = function() {   
			...
			for (j=0; j<controls.length; j++) { 
				...
				if (control === 'seek') { 
					...
					if (control === 'mute') { 
						...
						//JULIAN
						//ESTOY REUTILIZANDO EL BOTON DE PREFERENCIAS PARA ESTE CASO
					...
					if (control === 'captions') { 
						...
						if(!this.prefTrans || this.prefTrans !== 1){
							newButton.addClass('buttonOff').attr('title',this.tt.showTranscript);
						}
					}
					else if(control === 'sign'){
						if(!this.prefSign || this.prefSign !== 1) {
							newButton.addClass('buttonOff').attr('title', this.tt.showSign);
						}
					}
		...
		if (this.mediaType === 'video') { 
			...
			if(this.aspectRatio == '4:3' && this.captionsOn == true){ 
				//JULIAN
				//SI EL VIDEO ESTA EN FORMATO 4:3 ENTONCES DEJA UN ESPACIO EN NEGRO DEL TAMAÃ‘O DE DOS LINEAS DE TEXTO. 
				//DE ESTA FORMA LOS SUBTITULOS ENCIMA DEL VIDEO. 
				vidcapStyles = {
					'width': this.playerWidth+'px',
					'height': (this.playerHeight + 50)+'px'
				}
			}   
			else {
				vidcapStyles = {
		...
		//JULIAN
		//AQUI PARA FACILITAR EL LA COLOCACION DEL POPUP
		this.initDragDrop(this.$ableDiv);
...
(function ($) {		//properties and text format
...
(function ($) {		//text and audio description
	...
	AblePlayer.prototype.updateDescription = function (time) {
		...
		//JULIAN
		//DE ESTA FORMA LA AUDIO DESCRIPCION VA APARTE DE LA DESCRIPCION ESCRITA, POR LO QUE SE PUEDE PONER LA
		//AUDIO DESCRIPCION SIN PONER LA DESCRIPCION ESCRITA Y VICEVERSA. 
		if (this.textDescOn) {
	...
	AblePlayer.prototype.handleDescriptionClick = function(track){
		var thisObj = this;
		return function() {
			thisObj.item3.show();
			if(!thisObj.textDescOn){
				thisObj.textDescOn = true;
				thisObj.$descDiv.show();
				thisObj.$descDiv.removeClass('able-clipped');
			}
			thisObj.selectedDescriptions = track;
			
			thisObj.hidingPopup = true;     
			thisObj.descriptionsPopup.hide();
			thisObj.$descButton.focus();
			thisObj.refreshControls();
			thisObj.updateDescription();
		}
	};
	
	AblePlayer.prototype.toggleAudioDescription = function(){
		var thisObj = this;
		
		return function() {
			if(thisObj.descOn){
				thisObj.descOn = false;
				thisObj.item1.find("label").text(thisObj.tt.turnOnDescriptions);
			} 
			else {
				thisObj.descOn = true;
				thisObj.item1.find("label").text(thisObj.tt.turnOffDescriptions);
			}
			thisObj.hidingPopup = true;     
			thisObj.descriptionsPopup.hide();
			thisObj.$descButton.focus();
	...
	AblePlayer.prototype.textDescriptionOff = function() {
		var thisObj  = this;
		return function () {
			if(thisObj.textDescOn){
				thisObj.textDescOn = false;
				thisObj.item3.css("display", "none");
			} 
			thisObj.$descDiv.hide();
			thisObj.hidingPopup = true;     
			thisObj.descriptionsPopup.hide();
			thisObj.$descButton.focus();
			thisObj.refreshControls();
...
(function ($) {  	//time, states, and events
	...
	if(this.isFullscreen()){
		var descHeight = this.$descDiv.height();
		var controlHeight = this.$playerDiv.height();
		var height;
		if (this.textDescOn){
			height = (descHeight + controlHeight + 50);
		} 
		else {
	...
	AblePlayer.prototype.handleDescriptionToggle = function() { 
			if (this.descriptionsPopup.is(':visible')) {
				this.descriptionsPopup.hide();
				this.$descButton.focus();
			}
			else {
				this.closePopups();
				this.descriptionsPopup.show();
				this.descriptionsPopup.css('top', this.$ccButton.position().top - this.descriptionsPopup.outerHeight());
				this.descriptionsPopup.css('left', this.$ccButton.position().left);
				// Focus on the checked button, if any buttons are checked 
				// Otherwise, focus on the first button 
				this.descriptionsPopup.find('li').removeClass('able-focus');
				if (this.descriptionsPopup.find('input:checked')) { 
					this.descriptionsPopup.find('input:checked').focus().parent().addClass('able-focus');
				}
				else { 
					this.descriptionsPopup.find('input').first().focus().parent().addClass('able-focus');
				}
			}
	...
	AblePlayer.prototype.handleTranscriptToggle = function () {
		if (this.$accordionDiv.is(':visible')) {
			this.$accordionDiv.hide();
			this.$transcriptButton.addClass('buttonOff').attr('aria-label',this.tt.showTranscript);
			this.$transcriptButton.find('span.able-clipped').text(this.tt.showTranscript);
		}
		else {
			this.$accordionDiv.show();
	...
	AblePlayer.prototype.handleSignToggle = function () {
		if (this.$signWindow.is(':visible')) {
			...
			this.$signWindow.attr("aria-grabbed", "false");
	...
	AblePlayer.prototype.setFullscreen = function (fullscreen) {
		...
		if (this.nativeFullscreenSupported()) {
			...
			console.log("pantalla completa no estasoportado de forma nativa");
	...
	AblePlayer.prototype.getCaptionClickFunction = function (track) {
		...
		return function () {
			if(thisObj.captionsOn == false){
				var newHeight = thisObj.$vidcapContainer.height() +50;
				thisObj.$vidcapContainer.css("height", newHeight);
			}
...
(function ($) {		//json
	...
	AblePlayer.prototype.getDescriptionText = function (){
		var descriptionSRC = $("#video1").attr("data-desc-data-src");
		var thisObj = this;
		$.getJSON(descriptionSRC ,
		function(data) {
			thisObj.dt = data;
		});
	};

	AblePlayer.prototype.getDownloadText = function () {
		var downloadSRC = $("#video1").attr("data-download-src");
		var thisObj = this;
		$.getJSON(downloadSRC , 
		function (data) {
			thisObj.lt = data;
...
(function ($) { 	//media events, and click events
	// Media events
	...
	AblePlayer.prototype.onClickPlayerButton = function (el, e) {
		...
		else if (whichButton === 'options') {
			this.handleWindowButtonClick(e)
		}
	};
	...
	AblePlayer.prototype.addEventListeners = function () { 
		...
		this.$controllerDiv.find('button').click(function(e){
			thisObj.onClickPlayerButton(this, e);
		});
...
(function ($) {		//dragDrop and option menus
	...
	AblePlayer.prototype.initDragDrop = function ( $element ) {
		...
		//JULIAN
		//ESTOS CONDICIONALES DETECTAN QUE VENTANA HA LLAMADO A LA FUNCION 
		//Y LO PREPARAN DE FORMA QUE NO HAYA ERRORES
		if ($element.is('.able-sign-window')) { 
			this.windowName = 'sign-window';
		}
		else if ($element.is(".able")) {
			this.windowName = 'video-window';
		}
		else {
			console.log($element);
		}
	...
	AblePlayer.prototype.addWindowMenu = function() { 
		...
		// JULIAN
		// AHORA idAttr COJE EL NOMBRE AUTOMATICAMENTE DEPENDIENDO DE LA VENTANA QUE LO LLAMA
		var idAttr = this.windowName + '-button';
		var spanAttr = this.windowName + '-span';
		...
		//JULIAN
		//HAY UN BUG EN CHROME EN EL QUE EN VEZ DE DETECTAR QUE PULSES EL BOTON DETECTA QUE PULSAS EL SPAN QUE ESTA INCRUSTADO EN SU INTERIOR CON EL SIMBOLO.
		//POR LO QUE HAY QUE PONERLE AL SPAN UNA FORMA UNICA DE DETECTARLO, E INCLUIRLO EN LOS CONDICIONALES.
		if (this.iconType === 'font') {
			var $buttonIcon = $('<span>',{ 
		...
		if (this.windowName == 'sign-window') { 
			...
			options.push({
				//JULIAN
				//AÃ‘ADE LA OPCION PARA INCRUSTAR
				'name': 'embed',
				'label': this.tt.windowEmbed
			})
		...
		else if (this.windowName == 'video-window'){
			//JULIAN
			//AÃ‘ADE LAS OPCIONES AL MENU DE LA VENTANA PRINCIPAL.
			//SE AÃ‘ADE LA DE PREFERENCIAS YA QUE ESTAMOS USANDO EL BOTON DE PREFERENCIAS PARA LLAMARLO
			var options = [];
			options.push({
				'name': 'move',
				'label': this.tt.windowMove
		...
		if(this.windowName == 'video-window'){
			this.$optionsButton.parent().append($tooltip,$popup);
			//JULIAN
			//SE USA UNA CONDICION ESPECIFICA PARA ESTA VENTANA DADO QUE SI NO, CREA EL BOTON Y NO LO NECESITAMOS
			//YA QUE TENEMOS EL BOTON ORIGINAL DE PREFERENCIAS ESCUCHANDO SUS EVENTOS.
			var offset = this.$optionsButton.position();
			$popup.css({
				"top": offset.top,
				"right": this.$optionsButton.parent().width() - 12,
				"width": "9em"
			});
		}   
		else{
	...
	AblePlayer.prototype.addResizeDialog = function () { 
		...
		if(!thisObj.resizeDialog){ 
			//JULIAN
			//SI NO ESTA ESTO SE DUPLICA EL DIALOGO Y SE PROVOCAN INSESTABILIDADES
...
*/
			// JULIAN
			// BASICAMENTE AL RECIBIR EL CUADRO DE TEXTO EL FOCO
			// BORRA EL CONTENIDO, NO ES MAS QUE UNA MEDIDA PARA FACILITAR
			// TRABAJAR CON ELLO
			$resizeWidthInput.focus(function() {
				$resizeWidthInput.val("");
			})
			// Add save and cancel buttons.
			var $saveButton = $('<button class="modal-button">' + this.tt.save + '</button>');
			var $cancelButton = $('<button class="modal-button">' + this.tt.cancel + '</button>');
			$saveButton.click(function () {
				...
				if (newWidth !== startingWidth) { 
					...
					if(thisObj.$activeWindow.attr("class") === "able"){
						// DETECTA SI ESTAS TRABJANDO CON LA VENTANA DEL VIDEO PRINCIPAL
						// Y POR TANTO REDIMENSIONA SU CONTENIDO DE FORMA QUE TODO 
						// SEA ACORDE
						var $vidcapContainer = thisObj.$activeWindow.find('div.able-vidcap-container');
						$vidcapContainer.css("width", newWidth);
						$vidcapContainer.find('div').css("width", newWidth);
						$vidcapContainer.find('button').css("width", newWidth);
						$vidcapContainer.find('video').css("width", newWidth);
					}
				}
			...
			//JULIAN
			//ANTES AL HACER CLICK SE LLAMABA A "dialog.hide()" COSA QUE NO EXISTE
			//POR LO QUE LO HE MODIFICADO DE FORMA QUE SE OCULTE CORRECTAMENTE
			$cancelButton.click(function () {
				thisObj.resizeDialog.hide();
			});
	...
	AblePlayer.prototype.handleWindowButtonClick = function (e) { 
		...
		/* 		JULIAN
		ESTO FUNCIONA CON EL ULTIMO VALOR QUE LE HAS DADO A ESAS VARIABLES
		POR LO QUE SOLO AFECTAN AL VIDEO DE LENGUAJE DE SEÃ‘AS
		DEBERIA ENCONTRAR UNA FORMA DE QUE DETECTE SOBRE QUE VIDEO QUIERO TRABAJAR
		ESTE PROBLEMA ESTA RESUELTO EN LAS LINEAS INMEDIATAS
		CON ESTO DETECTA DESDE QUE VENTANA SE DISPARA. Y CAMBIA LO NECESARIO PARA QUE LO  SIGUIENTE FUNCIONE*/
		var target = $(e.target);
		if(target.is($(".able-button-handler-options")) || target.is("#Options-id")){
			this.$windowPopup = $("#video1-video-window-menu");
			this.$windowButton = $("#video-window-button");
			this.$activeWindow = $(this.$ableDiv);
		}
		else if ( target.is("#sign-window-button") || target.is("#sign-window-span")) {
			this.$windowPopup = $("#video1-sign-window-menu");
			this.$windowButton = $("#sign-window-button");
			this.$activeWindow = $(this.$signWindow);
		}
	...
	AblePlayer.prototype.handleMenuChoice = function ( choice ) { 
		...
		if (choice == 'move') { 
		...
		else if (choice == 'embed'){ //Para empotrar el video de lenguaje de seÃ±as, en el video principal
			if(!document.getElementById("video1").paused){
				// JULIAN
				// DETECTA SI EL VIDEO SE ESTABA REPRODUCIENDO PARA EVITAR
				// PROBLEMAS AL INCRUSTARLO
				var needPlay = true;
				document.getElementById("video1").pause();
				document.getElementById("video1-sign").pause();
			}
			//		Obtenemos la posicion y los tamaÃ±os de los dos videos. 
			var videoWidth = document.getElementById("video1").offsetWidth;
			var videoHeight = document.getElementById("video1").offsetHeight;
			var signWidth = document.getElementById("video1-sign").offsetWidth;
			var signHeight = document.getElementById("video1-sign").offsetHeight;
			var $newWidth = parseInt(videoWidth);
			$newWidth /= 2;
			this.videoSignOffset = this.$activeWindow.offset();
			var videoPos = $("#video1").offset();
			var opacity = 0.8;
			//		Cambia el ancho de la ventana del lenguaje de seÃ±as. 
			this.$activeWindow.css("width", $newWidth);
			this.$activeWindow.find("video").css("width", $newWidth);
			//		CALCULAR POSICION EN QUE QUEREMOS PONER EL VIDEO DE SEÃ‘AS
			var newTopPosition = videoPos.top;
			var newLeftPosition = videoPos.left;
			var newBottomPosition = videoPos.top + videoHeight ;
			var newRightPosition = videoPos.left + videoWidth ;
			//		Cambiar el video de seÃ±as a la columna del video principal
			var $ableSignWindow = $(".able-sign-window");
			$(".able-column-left").append($ableSignWindow);
			//		ASIGNAR NUEVA POSICION PRECALCULADA
			$ableSignWindow.css({
				"position": "absolute",
				"top": videoPos.top + (signHeight) - 50,
				"left": videoPos.left + (signWidth / 2),
				"margin": 0,
				"opacity": opacity
			});
			$ableSignWindow.css({
			...
			if(needPlay){
				// JULIAN
				// EN CASO DE QUE ESTUVIERA REPRODUCIENDOSE PREVIAMENTE VUELVE A REPRODUCIRSE
				document.getElementById("video1").play();
				document.getElementById("video1-sign").play();
			}
			var $thisRadio = $("#video1-sign-window-choice-2");
			$thisRadio.val("extract");
			$thisRadio.next("label").text(this.tt.windowExtract);
			this.$windowPopup.hide();
			this.$windowButton.show().focus();
			this.hidingPopup = false;
			this.popupOpen = false;
		}
		else if (choice == "extract"){//Sacar el video y devolverlo a su posicion original
			var $ableSignWindow = $(".able-sign-window");
			$ableSignWindow.appendTo($(".able-column-right"));
			$ableSignWindow.css({
				"position": "absolute",
				"top": this.videoSignOffset.top,
				"left": this.videoSignOffset.left,
				"width": 480,
				"opacity": 1,
				"margin": 1
			});
			...
			var $thisRadio = $("#video1-sign-window-choice-2");
			$thisRadio.val("embed");
			$thisRadio.next("label").text(this.tt.windowEmbed);
			this.$windowPopup.hide();
			this.$windowButton.show().focus();
			this.hidingPopup = false;
			this.popupOpen = false;
			...
			//JULIAN
			//PARA LANZAR EL MENU DE PREFERENCIAS YA QUE NO ESTA EL BOTON ORIGINAL PARA ESE PROPOSITO
			this.handlePrefsClick();
			this.$windowPopup.hide().parent().focus();
			this.hidingPopup = false;
			this.popupOpen = false;
	...
	AblePlayer.prototype.startDrag = function() { 
		...
		this.$activeWindow.addClass('able-drag').css({
			'position': 'absolute',
			/* 	JULIAN
				DE ESTA FORMA SE EVITA UN BUG AL INICIAR EL DRAG & DROP
				'top': this.dragStartY + 'px',
				'left': this.dragStartX + 'px' */
		});

		/*  JULIAN
		/*	POR ALGUN MOTIVO LA VENTANA DEL VIDEO PRINCIPAL NO GANA EL FOCO AUTOMATICAMENTE
		/*	POR LO QUE NO FUNCIONAN LAS TECLAS, ASI QUE SE LE ASIGNA AUTOMATICAMENTE */
...
(function ($) {		//sign language
	...
	AblePlayer.prototype.injectSignPlayerCode = function() { 
		...
		if (this.$ableColumnRight) { 
			this.$ableColumnRight.prepend(this.$signWindow);
		}
		else { 
